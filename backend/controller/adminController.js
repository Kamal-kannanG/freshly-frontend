...
exports.addFood = async (req, res) => {
  console.log("inside add fod")
  console.log(req.body)
  // var file = req.file
  let avail;
  let qty;
  let limit;
  if (!isNaN(req.body.groceryqty)) {
    if (req.body.groceryqty <= 0) {
      avail = false;
      qty = 0;
    } else {
      avail = true;
      qty = req.body.groceryqty;
    }
    if (req.body.groceryqty == -1) {
      avail = true;
      qty = -1;
    }
    // **********************
    try {
      ...
      var food = new Grocery({
        groceryname: req.body.groceryname,
        groceryqty: qty,
        groceryprice: req.body.groceryprice,
        // groceryimage: file.filename,
      ...
      try {
        ...
        io.emit("foodcrudbyadmin", " grocery crud operation done by admin!");
...
exports.editFoodWithImage = async (req, res) => {
  ...
  if (!isNaN(req.body.groceryqty)) {
    ...
    try {
      Grocery.findOne({ _id: req.body._id }, async (err, data) => {
        if (err) {
          ...
          if (!data) {
            ...
            try {
              // var x = await fileUploadmiddleware.deleteImage(data.groceryimage);
              // const image = req.file;
              // const imageUrl = await fileUploadmiddleware.uploadImage(image);
              Grocery.updateOne(
                { _id: req.body._id },
                ...
                function (err, item) {
                  if (err) {
                    ...
                    io.emit(
                      "foodcrudbyadmin",
                      " food crud operation done by admin!"
                    );
...
exports.deleteFood = (req, res) => {
  Grocery.findOne({ _id: req.params.id }, async (err, data) => {
    if (err) {
      console.log("error in delete food by admin");
      return res.json({ errormsg: "Somthing went wrong" });
    } else {
...
exports.getallOrders = (req, res) => {
  ...
  Order.find(
    {
      $or: [{ status: { $ne: "picked up" } }, { paymentstatus: "unpaid" }],
      orderdate: date,
    },
    (err, orders) => {
      if (err) {
  ...
  Order.find({ orderdate: date }, (err, orders) => {
    ...
    for (let i = 0; i < orders.length; i++) {
      ...
      for (let j = 0; j < element.length; j++) {
        let temp = {
          _id: element[j]._id,
          groceryqty: element[j].groceryqty,
          groceryprice: element[j].groceryprice,
          groceryname: element[j].groceryname,
          // groceryimage: element[j].groceryimage,
        };
        let k = 0;
        for (k = 0; k < emptyarray.length; k++) {
          if (emptyarray[k]._id == element[j]._id) {
            emptyarray[k].groceryqty += element[j].groceryqty;
            total += element[j].groceryqty * element[j].groceryprice;
            break;
          }
        }
...
exports.getQrcode = (req, res) => {
  ...
  Order.findOne({ _id: id }, (err, order) => {
    ...
    if (order.status == "completed" || order.status == "picked up") {
      ...
      console.log(
        "order status must be completed or pickup for getting QR code"
      );
      return res.json({ errormsg: "error while generating qr code" });
    }
  });
