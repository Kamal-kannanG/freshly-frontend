  ...
  User.updateOne(
    ...
    function (err, user) {
      if (err) {
        ...
        res
          .status(201)
          .json({ msg: "edited profile", emailchange: emailchange });
      }
    }
  );
};
...
exports.getallFoodItem = (req, res) => {
  Grocery.find({ groceryavail: true }, (err, items) => {
    if (err) {
...
function decrementQuantity(req, res, id) {
  Grocery.findOne({ _id: id }, (error, item) => {
    if (error) {
      console.log("something went wrong!!");
      res.json({ errormsg: "something went wrong!!" });
    } else {
      var avail = true;
      let qty = item.groceryqty;
      if (qty - 1 == 0) {
        avail = false;
      }
      Grocery.updateOne(
...
function secondtimecart(req, res, oldcart, newitem) {
  var oldavail = false;
  var newtotal = oldcart.total + newitem.groceryprice;
  var tot;
  ...
  if (oldavail) {
    ...
    for (var i = 0; i < olditemsjsonarray.length; i++) {
      if (olditemsjsonarray[i]._id == newitem._id) {
        olditemsjsonarray[i].groceryqty += 1;
        oldcart.total += olditemsjsonarray[i].groceryprice;
        tot = oldcart.total;
      }
    }
    Cart.updateOne(
      ...
      if (!cart) {
        ...
        for (var i = 0; i < olditemjsonarray.length; i++) {
          if (olditemjsonarray[i]._id != req.body._id) {
            ...
            cart.total -= req.body.groceryprice * req.body.groceryqty;
            total = cart.total;
          }
        }
        // console.log(cart);
...
async function SaveinOrder(req, res, cart) {
  ...
  for (let i = 0; i < allitems.length; i++) {
    ...
    const oneitemqty = oneitem.groceryqty;
    await Grocery.findById(oneitemid, (err, orignalitem) => {
      if (err) {
        console.log("something went wrong!!");
        res.json({ errormsg: "something went wrong!!" });
      } else {
        if (!orignalitem.unlimited) {
          const orignalitemqty = orignalitem.groceryqty;
          if (orignalitemqty - oneitemqty < 0) {
            errormessage += " " + orignalitem.groceryname;
          }
        } else {
          if (!orignalitem.groceryavail) {
            errormessage += " " + orignalitem.groceryname;
          }
        }
      }
  ...
  if (errormessage != "") {
    ...
    for (let i = 0; i < allitems.length; i++) {
      ...
      await Grocery.findOne({ _id: oneitemid }, async (err, onefooditem) => {
        if (err) {
          console.log("something went wrong!!");
          res.json({ errormsg: "something went wrong!!" });
        }
        if (!onefooditem) {
          ...
          if (!onefooditem.unlimited) {
            let avail = true;
            if (onefooditem.groceryqty - oneitemqty <= 0) {
              avail = false;
            }
            await Grocery.updateOne(
              { _id: oneitemid },
              {
                groceryqty: onefooditem.groceryqty - oneitemqty,
                groceryavail: avail,
              },
              function (err, done) {
                if (err) {
      ...
      if (order.status == "completed") {
        Order.updateOne(
          ...
          (err, done) => {
            if (err) {
              console.log(
                "error while scanning qr code and updating status of by user"
              );
              return res.json({ errormsg: "Somthing went wrong" });
            } else {
